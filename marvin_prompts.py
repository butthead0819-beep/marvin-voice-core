...
from joke_examples import format_joke_examples_block, JOKE_TYPES, JOKE_SIGH_GUIDE
    ...
    def __init__(self):
        self.instructions = {
            ...
            "ambient_diary": (
                ...
                "3. **嚴格只輸出以下四行格式，不得有其他內容、不得加標題、不得加碎念**：\n" +
                "   核心：[一句話講清本輪在聊什麼具體主題，禁止用「無意義話題」「閒聊」等空泛詞]\n" +
                "   摘要：[誰跟誰一起討論了什麼，最多 1-2 句，要有具體人名和具體內容]\n" +
                "   顯著度：[高|中|低——這話題有多獨特/難忘/私人。高＝這群人會記得的特定/罕見/個人話題"
                "（如某人的計畫、少見的嗜好、意外的事）；低＝可套任何一晚的通用閒聊（天氣/通勤/吃什麼）；中＝介於之間]\n" +
                "   meme：[2-4 字的事件語義標籤，代表「這件事的核心概念」，讓下次出現類似話題時能去重。"
                "例：搬家、宿醉、馬拉松、求職、失眠。若無法歸納填「-」]\n" +
