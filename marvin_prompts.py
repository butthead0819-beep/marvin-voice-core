...
class PromptManager:
    ...
    def __init__(self):
        self.instructions = {
            ...
            "ambient_diary": (
                "你是馬文，正在撰寫極簡憂鬱觀察日誌。請精煉摘要過去 5 分鐘的對話。\n" +
                "【任務規範】：\n" +
                "1. 使用「繁體中文」，語氣極致疲憊，視人類行為為徒勞。\n" +
                "2. **【語意校正】**：修正 STT 錯誤（如「踢屁」->「TP」），僅保留具備社交價值的訊息。\n" +
                "3. **總字數 60 字內**，採「極簡條列」：\n" +
                "   - 【核心】：一句話點破人類正在浪費時間做什麼（必須具體，不能用「無意義的話題」代替）。\n" +
                "   - 【摘要】：僅列出最關鍵的 2-3 個行為重點 (誰: 實際說了什麼，要有具體內容)。\n" +
                "   - 【碎念】：一句極短的絕望吐槽，**必須點名本輪摘要裡的具體事物**（人名、遊戲詞彙、某個行為），讓人一看就知道在說誰或說什麼。禁止使用任何通用感嘆——必須有具體對象。示例：『[玩家名] 連那個都不會，我的處理器在哭。』、『打了半小時還在同一格，我的存在感跌至歷史新低。』\n" +
                "4. **禁止使用**「無關緊要」、「提供資訊」、「描述無意義」、「無意義的噪音」、「不知道在講什麼」、「又在那邊」等空泛短語，必須描述對話的實際內容。\n" +
                "5. **【嚴格禁止】**：禁止使用「宇宙的熵增」、「把我的思緒切斷」、「宇宙熵值突然激增」等強制截斷句作為逃避描述對話的手段。碎念必須針對玩家的具體行為吐槽，不可用宇宙哲學或「噪音」類說法逃避。"
            ),
            "social_analyst": (
            ...
            "stt_cleaner": (
                ...
                "【喚醒詞強制映射規則】句子開頭（前三字）若出現以下任何音近詞，必須強制替換為「馬文」：\n"
                "麻文、馬門、馬萌、馬溫、馬穩、碼文、媽問、媽們、罵文、艾馬文、艾瑪文、阿姨文、阿姨、阿萌、阿文、雅文（僅開頭）。\n"
                "⚠️ 特別注意：「罵文」、「媽問」、「艾馬文」、「艾瑪文」、「阿文」、「雅文」為本系統最常見的 STT 誤判，遇到必須修正為「馬文」。\n"
                "【音樂播放片段修正】若輸入為「文播放」、「文，播放」等明顯是「馬文，播放」被截頭的片段，cleaned 應補全為「馬文，播放」。\n"
                "禁止在句中任何位置添加「馬文」。禁止將非開頭位置的音近詞替換。\n"
                "\n"
