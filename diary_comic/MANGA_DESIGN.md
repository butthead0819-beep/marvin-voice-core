...
### 1. 切格大小 = 強弱與節奏（最重要）
- **一頁只能有一個最大、最重要的格（大砸框）≥40% 面積**（Jack 2026-06-21）。
- 先決定這頁的「高潮點是哪一幕」（我們=爆笑反應那一幕），全頁 40%+ 留給它；**其餘格都為它鋪陳**。
- 現代漫畫（火影/海賊）約 3-4 格/頁，emphasis 驅動，不是塞滿。
- ✅ 已做：`layout.splash_layout`（高潮底部≥40%）+ `compose_splash_page`（高潮砸框+上方鋪陳小格）。
  story 路徑：高潮=爆笑反應(情緒特寫砸框)、鋪陳=物件context + 鋪哏setup 小格。
...
└──────────────────────┘
```
- 拆兩拍（Ⓐ）保留但配合砸框：**鋪哏 setup = 小鋪陳格**、**爆笑 reaction = 大砸框**（≥40%）。
  setup→punchline 節奏在，且符合「一頁一個大格」。
- arc（②B）：高潮(砸框)在底，鋪陳在上，往下讀到爆點。
