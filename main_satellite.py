...
async def setup_browser_satellite(bot):
    """純軟體 satellite wiring：載 cog + 綁 BrowserSpeakerOutput 輸出（不連 Pi）。

    回 (vc, browser_out)；browser_out 給 build_text_app 當 reply_source（GET /reply）。
    """
...
def build_text_app(vc, *, token: str | None = None, default_speaker: str = "狗與露",
                   reply_source=None, car_presence=None):
...
async def start_text_http_server(vc, reply_source=None):
...
async def main():
    ...
    async with bot:
        ...
        if os.getenv("MARVIN_SATELLITE_BROWSER", "").strip().lower() in ("1", "true", "yes", "on"):
            vc, browser_out = await setup_browser_satellite(bot)
            ...
            await start_text_http_server(vc, reply_source=browser_out)
