...
class PlaybackDevice(Protocol):
    ...
    def arm_mixer(self, source: Any, *, after=None) -> None:
        """啟動持續性 mixer 播放。Discord=vc.play(opus+bitrate)、Local=起本機泵。

        after：播放器（AudioPlayer thread / 本機泵）自然結束時的 callback(error)。
        Discord 端用它接自癒重武裝——見 DiscordPlaybackDevice.arm_mixer。
        """
        ...
