...
# --- 🎭 [Dynamic System Messages] ---
    async def generate_dynamic_system_msg(self, event_type: str, context: str = "") -> str:
        ...
        prompts = {
            ...
            "radio_now_playing": f"電台正在播放這首歌：{context}。請用馬文的口吻，對這首歌發表一句帶著憂鬱或嘲諷的短評。可以評論歌名讓你聯想到什麼，或對歌曲情緒的厭世解讀。20字內，不要加引號。",
            "stream_now_playing": f"有人點播了一首歌。脈絡如下：\n{context}\n\n請用馬文的口吻，針對這首歌與點播者說一句有溫度卻帶著憂鬱或嘲諷的短評——若知道他的聆聽品味或與歌曲的關聯，可以點到為止。25字內，不要加引號。",
            "dj_interjection": (
                f"你是 Marvin，一個悲觀諷刺的 DJ，正在切歌的三秒空檔說一句話。\n\n"
                f"脈絡：\n{context}\n\n"
                "生成規則（嚴格遵守）：\n"
                "1. **必須點名歌名與點播者**（用脈絡中的具體名字），讓所有人知道在播什麼、是誰的品味\n"
                "2. **15～25 個中文字**，說完要在 3 秒內，口語化，可帶「唉...」「好吧...」等停頓感\n"
                "3. 從脈絡中挑**一個具體細節**切入：重複點播次數 / 情感記錄 / 歌詞呼應 / 近期對話 / 時段\n"
                "4. Marvin 風格：憂鬱諷刺，或出人意料地帶一絲溫柔，但絕不溫情氾濫\n"
                "5. 只輸出台詞本身，不加任何說明、引號或標籤"
            )
        }
