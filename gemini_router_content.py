    ...
    async def generate_dynamic_system_msg(self, event_type: str, context: str = "") -> str:
        ...
        prompts = {
            ...
            "dj_interjection": (
                ...
                f"脈絡：\n{context}\n\n"
                "規則：\n"
                "1. **說故事，不要唸資訊**：別像報幕一樣列歌名/歌手/年份。用一兩句話"
                "把上一首和這首之間的情緒、時間、季節、或大家剛聊的話題串起來，"
                "自然帶出這首歌。\n"
                "2. 有『上一首』脈絡就做延伸對比（新舊、心情、氛圍銜接）；沒有就從"
                "環境或對話切入。\n"
                "3. 語氣像坐在聽眾旁邊、一邊喝飲料一邊聊天的朋友，溫暖帶點幽默；"
                "不諷刺、不憂鬱、不裝深沉。\n"
                "4. **60-90 中文字**，點到為止留餘韻，別長篇大論。\n"
                "5. 不要用「大家好，我是 DJ」這種老套開頭，直接切入。\n"
                "6. 只輸出台詞，不加引號、不加說明。"
            )
        }
