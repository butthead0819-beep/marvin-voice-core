...
_CURATION_SYS = (
    ...
    "既新鮮、又顯得你真的懂他們。寧可挖深也不要重播大熱門。\n"
    "4) 歌單要連貫——同一種年代/情緒/語言掛在一起，不是各自為政。\n"
    "5) 每首給一句『選歌理由』：像朋友跟你說為什麼放這首、扣回今晚的主題，短、有人味。\n"
    "6) 只選真實存在的歌（真歌手＋真歌名），不確定就不要編。\n"
...
def build_curation_prompt(brief: ThemeBrief, exclude_titles: list[str], *,
    ...
    # 放寬到 80 首：原本只給前 40 → LLM 看不到後面的已播歌就重選（新鮮度漏）。
