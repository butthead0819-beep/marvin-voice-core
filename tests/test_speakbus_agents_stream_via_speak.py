...
def _mk_ctrl(mem, history=None, *, stream_mode=False):
    """Minimal controller stub；可 toggle stream_mode 模擬 voice 狀態。"""
    ctrl = MagicMock()
...
def _mk_ctx(present_speakers, last_speaker=None):
    return SpeakContext(
...
# ── 1. MemoryCallback speak_bid stream_mode gate ────────────────────────────


@pytest.mark.asyncio
async def test_memory_callback_bid_returns_zero_in_stream_mode(monkeypatch, tmp_path):
    """stream_mode=True → dense 0.0 with reason='stream_mode'（看齊既有 agent）。"""
    ...
    agent = MemoryCallbackAgent(
        _mk_ctrl(mem, history=history, stream_mode=True),
...
async def test_memory_callback_bid_still_works_when_not_in_stream(monkeypatch, tmp_path):
    ...
    history = [_utt("Alice", "grounded search 那個怎樣")]
    agent = MemoryCallbackAgent(
        _mk_ctrl(mem, history=history, stream_mode=False),
        confidence=0.7, overlap_threshold=0.3,
    )
...
async def test_memory_callback_speak_uses_speak_helper(monkeypatch, tmp_path):
    ...
    ctrl = _mk_ctrl(mem, history=history, stream_mode=False)
