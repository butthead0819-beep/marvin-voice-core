...
（fetch_car_now_title()），變化時才觸發 on_track_change，不是像舊版那樣
...
def test_title_poll_calls_on_track_change_when_title_changes(monkeypatch):
    ...
    monkeypatch.setattr("device.puck_mixer.fetch_car_now_title", lambda: "測試歌")
    monkeypatch.setattr("device.puck_mixer._TITLE_POLL_INTERVAL_S", 0.01)

    mixer._ensure_title_poll_running()
    assert _wait_until(lambda: calls == ["測試歌"])
...
def test_title_poll_none_title_does_not_call_hook(monkeypatch):
    """fetch_car_now_title() 回 None（沒在播/連不到 Mac）→ 不觸發、不清掉現有 title。"""
    calls = []
    mixer = PuckMixer(bt_mac="AA:BB:CC:DD:EE:FF", on_track_change=calls.append)
    monkeypatch.setattr("device.puck_mixer.fetch_car_now_title", lambda: None)
    monkeypatch.setattr("device.puck_mixer._TITLE_POLL_INTERVAL_S", 0.01)
