...
def curate_reel(rows, song_requests=None, bin_s: float = BIN_S,
                max_topics: int = MAX_TOPICS) -> Reel | None:
    """rows → Reel，或 None（無人說話 / 無有主題搶話 → 退靜態海報）。

    底層曲線＝整晚發言密度（熱鬧弧線）。紅點只標「有主題」的搶話（事件含過品質閘的句，
    濾掉純嗨/糊字），每晚最多 max_topics 個、取最熱。hero＝最熱有主題事件（freeze+引言）。
    songs＝(ts,點歌者,歌名)，裁到整晚窗內、標在時間軸上。
    """
...
_MARK = (235, 110, 110)      # 有主題搶話紅點
_HERO = (255, 220, 120)      # hero freeze 點
_SONG = (120, 210, 150)      # 點歌標記 ♪
_TXT = (235, 235, 235)
...
def render_ekg_png(reel: Reel, out_path: str) -> str:
    ...
    for ts, _h in reel.topic_peaks:
        ...
        d.ellipse([mx - 5, my - 5, mx + 5, my + 5], fill=_MARK)
    ...
    # 點歌標記：時間軸下方一條 lane，每首歌打 ♪ + 垂直 tick
    song_lane = plot_bot + 18
    ...
    d.text((_MARGIN, 52), getattr(reel, "date", "") or "", font=f_small, fill=_TXT)
    d.text((_MARGIN, plot_bot + 48), f"「{reel.quote}」", font=f_quote, fill=_TXT)
    d.text((_MARGIN, plot_bot + 104), "— " + "、".join(reel.speakers),
           font=f_small, fill=(170, 170, 175))

    # 點歌清單（時間 + 歌名 + 點歌者），最多 5 行、超過標 +N
    if reel.songs:
        from diary_comic.song_requests import clean_title
        ly = plot_bot + 150
        d.text((_MARGIN, ly), "🎵 今夜點歌", font=f_song, fill=_SONG)
        ly += 32
        shown = reel.songs[:5]
        for ts, u, t in shown:
            hhmm = _dt.datetime.fromtimestamp(ts).strftime("%H:%M")
            d.text((_MARGIN, ly), f"{hhmm}  {clean_title(t)}  — {u}",
                   font=f_song, fill=(190, 190, 195))
            ly += 30
        extra = len(reel.songs) - len(shown)
        if extra > 0:
            d.text((_MARGIN, ly), f"…＋{extra} 首", font=f_song, fill=(150, 150, 155))
