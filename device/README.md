...
| `temp_guard.sh` | `/usr/local/bin/` | SoC 溫度防護：過熱→本機靜音 DigiAMP（amixer Digital mute，見下方限制） |
...
是**安全網不是根治**。門檻（`MARVIN_TEMP_STOP_C`，預設 **50°C**，取較大裕度及早砍發熱；離家 idle 時 amp 本來就靜音故無害，
在家待機 idle≈55 會被觸發、播歌前需先 `marvin-mic amp on`）。觸發動作＝本機 `amixer Digital mute`（黏著、不自動解除；
冷卻後 `marvin-mic amp on` 手動解除）。CLEAR 預設 48。根治仍是散熱片＋24V/5A。
