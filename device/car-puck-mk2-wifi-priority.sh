...
#   - 下面三個變數用實機查出來的值填入；BMW_MAC 應該跟
#     car-puck-mk2-btspk-autoconnect.sh 的 __CAR_HEAD_UNIT_MAC__ 是同一顆。
HOME_WIFI_CON=__HOME_WIFI_CON_NAME__
...
nmcli connection modify "$HOME_WIFI_CON" connection.autoconnect-priority 10 2>/dev/null
...
_home_wifi_active() {
    nmcli -t -f NAME connection show --active 2>/dev/null | grep -qx "$HOME_WIFI_CON"
}

while true; do
    if _bmw_connected; then
        # 人在車上 → 家用WiFi 不該連：關掉 autoconnect，且若正連著就主動斷開
        nmcli connection modify "$HOME_WIFI_CON" connection.autoconnect no 2>/dev/null
        if _home_wifi_active; then
            nmcli connection down "$HOME_WIFI_CON" >/dev/null 2>&1
        fi
    else
        # 沒連 BMW（在家 / 剛開機還沒配對上）→ 恢復家用WiFi 正常自動連線
        nmcli connection modify "$HOME_WIFI_CON" connection.autoconnect yes 2>/dev/null
    fi
    sleep 15
done
