...
class GroqAgent(LLMAgent):
    ...
    def bid(self, ctx: LLMContext) -> LLMBid:
        # 🚦 [急迫性硬分流] 背景/可延後 purpose（記憶挖掘、社交分析、摘要…）不佔 Groq。
        # Groq 最稀缺、有每日 TPD 上限，是 429 cooldown 的主來源 → 完整保留給阻塞即時回應。
        # 直接 decline（非軟降權）→ 逼去 Cerebras（近無限 RPM）/ gemini-free / 付費兜底。
        if ctx.purpose in BACKGROUND_PURPOSES:
            return LLMBid(
                confidence=0.0, provider="groq", model="?",
                estimated_latency_ms=0, estimated_cost_units=0,
                reason="deferrable:reserve_groq_for_realtime",
            )
