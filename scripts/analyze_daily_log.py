...
- 計算今日各類反應的數量與比例
- 計算品質分數（0-10）：
  `score = (喜歡×2 + 提出興趣×1 - 錯誤×1 - 嚴重×2) / total × 10`，clamp 至 [0, 10]
- 與 `suki_memory.json` 中昨日分數比較，判斷趨勢
- 歸納「問題模式」：哪些情境/話題容易得到嚴重/錯誤？
- 歸納「成功模式」：哪些回應風格容易得到喜歡/提出興趣？

### 步驟 5：Prompt 改善建議
根據問題模式，提出具體的 Prompt 修改方向，指明對應的 Prompt 名稱（`fast_awakening`、`ambient_diary`、`stt_cleaner` 等）
