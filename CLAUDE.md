...
### 5. 收尾一行結論（給 Marvin HUD 機讀）

**完成一輪任務、把控制權交還使用者前，回應的最後一行必須是這個格式：**

`🏁 <15字內：處理了什麼問題> — <30字內：結果或下一步>`

- 標題講「處理了什麼問題」，不要講「怎麼做的」（例：`🏁 修登入閃退` 而非 `🏁 改了 auth.py`）。
- 這行放在回應最後一行，不要塞在中間或被後續文字蓋過。
- Marvin HUD 的 Claude Code 狀態卡直接讀這一行當標題／內文，不會二次 LLM 摘要，所以這行本身就要是最終結論（見 `scripts/scan_claude_sessions.py`）。
- 純聊天、還在釐清需求、被使用者打斷的回合可以省略——沒有「處理了什麼問題」可講就不用硬湊。
